...
endpoint. `upstream-cdc-wdm` is the preferred GSM profile; `upstream-vodafone` is a
fallback.
...
Given that, `upstream-cdc-wdm` uses provider auto-configuration (`auto-config=true`) as
the conservative default. We keep the explicit `upstream-vodafone` profile as a fallback
because a Vodafone SIM was finnicky in the past and needed a specific APN
(`wap.vodafone.co.uk`); the EE- and TalkMobile-specific profiles were dropped since
auto-config covers them.
...
- `etc_files_pi/upstream-cdc-wdm.nmconnection.template` - preferred generic GSM
  ...
  `${MODEM_IFACE}`/`${MODEM_ID}` as `upstream-cdc-wdm0` through `upstream-cdc-wdm7`.
